...
# 0. Purpose of the document and initial set-up

* **Scripts** — Provide the scripts used for the clinical analysis regarding the clinical utility of MRD in HNSCC patients in Althoff et al (2026).
* **Clinical Analysis output** — Provide the raw outputs of the clinical analysis and figures used in the manuscript.
...
* **Flow chart** — Flow chart depicting ctDNA results by stage at baseline and in the MRD and post-definitive treatment surveillance windows
* **Demographics Tables** — Tables with Patient and tumor characteristics (Table 1 & Supplementary Table 1)
...
Overview plot depicting treatment regimen, longitudinal ctDNA results, and clinical outcome for all patients in the cohort, grouped by clinical stage. 
...
# 3. Miscellaneous cohort characteristics and analysis

* **Overall survival metrics for cohort** — PFS and OS estimates for complete cohort (n = 97)
* **ctDNA time points and imaging assessment cadence** — Median of median interval of ctDNA timepoints and radiological imaging assessment
...
Kaplan-Meier estimates of outcomes stratified by ctDNA at MRD status
...
Kaplan-Meier estimates of PFS stratified by ctDNA status in the MRD window among patients with HNSCC who did not receive any adjuvant therapy after surgery/definitive treatment
...
Kaplan-Meier estimates of PFS and OS stratified by ctDNA status in the MRD window among patients with:

* **Stage I/II and III/IV groups** — PFS and OS estimates
* **p16 status (negative and positive)** — PFS and OS estimates
* **MRD window analysis in patients treated with chemoRT or surgery** — PFS and OS estimates. Due to low numbers (27% of overall MRD cohort), KM plots for PFS for MRD time point after surgery was not reported in the figure, but outlined in the text.
...
circ_data <- circ_data[circ_data$ctDNA.available=="TRUE",]
circ_data <- circ_data[circ_data$ctDNA.MRD!="",]
circ_data <- circ_data[circ_data$MRD.Group=="ChemoRT",]
circ_datadf <- as.data.frame(circ_data)
...
surv_object <-Surv(time = circ_data$PFS.months, event = circ_data$PFS.Event)
KM_curve <- survfit(surv_object ~ ctDNA.MRD, data = circ_data,conf.int=0.95,conf.type="log-log") 
ggsurvplot(KM_curve, data = circ_data, pval = FALSE, conf.int = FALSE, risk.table = TRUE, break.time.by=6, palette=c("blue","red"), title="PFS - ctDNA at MRD chemoRT", ylab= "Progression-Free Survival", xlab="Months from Definitive Treatment", legend.labs=c("ctDNA Negative", "ctDNA Positive"), legend.title="")
summary(KM_curve, times= c(0, 12, 24, 36))
circ_data$ctDNA.MRD <- factor(circ_data$ctDNA.MRD, levels=c("NEGATIVE","POSITIVE"))
cox_fit <- coxph(surv_object ~ ctDNA.MRD, data=circ_data) 
ggforest(cox_fit,data = circ_data) 
...
surv_object <-Surv(time = circ_data$OS.months, event = circ_data$OS.Event)
KM_curve <- survfit(surv_object ~ ctDNA.MRD, data = circ_data,conf.int=0.95,conf.type="log-log") 
ggsurvplot(KM_curve, data = circ_data, pval = FALSE, conf.int = FALSE, risk.table = TRUE, break.time.by=6, palette=c("blue","red"), title="OS - ctDNA at MRD chemoRT", ylab= "Overall Survival", xlab="Months from Definitive Treatment", legend.labs=c("ctDNA Negative", "ctDNA Positive"), legend.title="")
summary(KM_curve, times= c(12, 24, 36))
circ_data$ctDNA.MRD <- factor(circ_data$ctDNA.MRD, levels=c("NEGATIVE","POSITIVE"))
cox_fit <- coxph(surv_object ~ ctDNA.MRD, data=circ_data) 
ggforest(cox_fit,data = circ_data) 
...
```{r}
# **PFS by ctDNA at MRD post-surgery**
...
circ_data <- circ_data[circ_data$ctDNA.available=="TRUE",]
circ_data <- circ_data[circ_data$ctDNA.MRD!="",]
circ_data <- circ_data[circ_data$MRD.Group=="Surgery",]
circ_datadf <- as.data.frame(circ_data)
...
median_fu <- survfit(Surv(time = circ_data$OS.months, event = 1 - circ_data$OS.Event) ~ 1, data = circ_data)
print(median_fu)
```

# 8. Figure 5

Kaplan-Meier estimates of outcomes stratified by ctDNA in surveillance and role of ctDNA dynamics.

* **ctDNA status in post-definitive treatment surveillance** — PFS and OS estimates, time-dependent and Multivariate analysis.
* **MRD dynamics in surveillance** — Serial ctDNA measurements for patients grouped based on ctDNA status in the post-definitive treatment surveillance window,
* **Lead time calculations and ctDNA velocity** — Lead time in months is calculated between molecular recurrence (ctDNA positivity, indicated by the blue dot) and clinical recurrence (radiographic imaging positive, indicated by the red dot) during the post-definitive treatment surveillance window (N = 22). Linear regression lines of ctDNA levels at the time of clinical recurrence.
...
Kaplan-Meier estimates of PFS and OS stratified by ctDNA status in post-definitive treatment surveillance among patients with:
...
Kaplan-Meier estimates of PFS stratified by ctDNA status among patients with:

* **MRD and surveillance timepoints available** — PFS and OS estimates
* **Landmark of 3mo timepoint after definitive treatment** — PFS and OS estimates
...
```{r}
# **PFS by ctDNA status at 3 months landmark**

rm(list = ls())

## =========================================================================
## STEP 1. Identify the 3-month landmark patients (from the OP dataset)
##         62-118 days between ctDNA draw and end of treatment,
##         and carry forward the landmark timepoint's date offset.
## =========================================================================
op <- read.csv("CLIA HNSCC UNM_OP.csv", stringsAsFactors = FALSE)
op$StartDate <- as.Date(op$StartDate, format = "%m/%d/%Y")
op$EndDate   <- as.Date(op$EndDate,   format = "%m/%d/%Y")

## End-of-treatment date per patient (chemoradiation or radiotherapy)
tx <- op[op$Tx_type %in% c("Chemoradiotherapy", "Radiotherapy") &
           !is.na(op$Tx_type), ]
tx_end <- aggregate(EndDate ~ PatientName, data = tx, FUN = max)
names(tx_end)[2] <- "TxEndDate"

## ctDNA timepoints, days from end of treatment
ctdna <- op[op$Event_type %in% c("ctDNA_pos", "ctDNA_neg") &
              !is.na(op$Event_type), ]
ctdna <- merge(ctdna, tx_end, by = "PatientName", all.x = TRUE)
ctdna$days_from_tx <- as.numeric(ctdna$StartDate - ctdna$TxEndDate)

## Landmark flag: POSITIVE / NEGATIVE within the 62-118 day window
ctdna$ctDNA.landmark.3mo <- with(ctdna, ifelse(
  !is.na(days_from_tx) & days_from_tx >= 62 & days_from_tx <= 118,
  ifelse(Event_type == "ctDNA_pos", "POSITIVE", "NEGATIVE"),
  NA
))

## Keep only in-window draws, and pick the EARLIEST in-window draw per patient
## as the defining landmark timepoint. Carry its date.diff (in DAYS).
ctdna_lm <- ctdna[!is.na(ctdna$ctDNA.landmark.3mo), ]
ctdna_lm <- ctdna_lm[order(ctdna_lm$PatientName, ctdna_lm$days_from_tx), ]
ctdna_lm <- ctdna_lm[!duplicated(ctdna_lm$PatientName), ]

landmark <- ctdna_lm[, c("PatientName", "ctDNA.landmark.3mo", "date.diff")]
names(landmark) <- c("PatientID", "ctDNA.landmark.3mo", "landmark.date.diff.days")

cat("Patients identified:", length(unique(landmark$PatientID)), "\n")

## =========================================================================
## STEP 2. Pull PFS.Event / PFS.months from the clinical dataset for
##         exactly those patients, using the landmark call as the group.
## =========================================================================
circ_data <- read.csv("CLIA HNSCC Peddada Clinical Data.csv",
                      stringsAsFactors = FALSE)

## Bring in the landmark grouping + date offset (overwrite empty placeholder)
circ_data$ctDNA.landmark.3mo <- NULL
circ_data <- merge(circ_data, landmark, by = "PatientID", all.x = TRUE)

## Keep only the identified landmark patients
circ_data <- circ_data[!is.na(circ_data$ctDNA.landmark.3mo), ]
circ_data$ctDNA.landmark.3mo <- factor(circ_data$ctDNA.landmark.3mo,
                                       levels = c("NEGATIVE", "POSITIVE"))

## -------------------------------------------------------------------------
## LANDMARK SHIFT: convert the landmark timepoint offset from DAYS to MONTHS
## and subtract it from PFS.months so time is measured from the landmark.
## -------------------------------------------------------------------------
circ_data$landmark.time.months <- circ_data$landmark.date.diff.days / 30.4375
circ_data$PFS.months.landmark  <- circ_data$PFS.months - circ_data$landmark.time.months

## Landmark logic: drop anyone who progressed/was censored before the landmark
## (would produce a negative shifted time).
n_before <- nrow(circ_data)
circ_data <- circ_data[!is.na(circ_data$PFS.months.landmark) &
                         circ_data$PFS.months.landmark >= 0, ]
cat("Dropped for negative post-landmark PFS:", n_before - nrow(circ_data), "\n")

## =========================================================================
## STEP 3. Survival analysis (using landmark-shifted PFS)
## =========================================================================
event_summary <- circ_data %>%
  group_by(ctDNA.landmark.3mo) %>%
  summarise(
    Total      = n(),
    Events     = sum(PFS.Event),
    Fraction   = Events / n(),
...
survfit(Surv(time = circ_data$PFS.months.landmark, event = circ_data$PFS.Event)~ctDNA.landmark.3mo, data = circ_data)
surv_object <- Surv(time = circ_data$PFS.months.landmark, event = circ_data$PFS.Event)

KM_curve <- survfit(surv_object ~ ctDNA.landmark.3mo, data = circ_data,
                    conf.int = 0.95, conf.type = "log-log")

ggsurvplot(KM_curve, data = circ_data, pval = FALSE, conf.int = FALSE,
           risk.table = TRUE, break.time.by = 6,
           palette = c("blue", "red"),
           title = "PFS - ctDNA 3-Month Landmark",
           ylab = "Progression-Free Survival",
           xlab = "Time from landmark timepoint (months)",
           legend.labs = c("ctDNA Negative", "ctDNA Positive"),
           legend.title = "")

summary(KM_curve, times = c(0, 12, 24, 36))

cox_fit <- coxph(surv_object ~ ctDNA.landmark.3mo, data = circ_data)
ggforest(cox_fit, data = circ_data)
summary(cox_fit)
...
label_text <- paste0("HR = ", round(HR, 2),
...
Two examples of illustrative cases that demonstrate the role of serial Signatera assessment in patient with HNSCC.
